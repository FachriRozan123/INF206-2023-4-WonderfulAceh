 ...
 switch (state) {
   case "connected":
    if (net_errs < 1) {
      ...
      // Ubah gaya CSS langsung di dalam JavaScript
 ...
 if (!searchLoading && !noMoreDataSearch) {
   ...
   $.ajax({
     ...
     success: (data) => {
       ...
       // Add click event listener to user search results
       $(".search-records .user-item").on("click", function() {
         // Get the user ID from the data attribute
         var userId = $(this).data("user-id");

         // Redirect to the user's ID-based URL
         window.location.href = url + "/user/" + userId;
       });
     },
     error: (error) => {
       setSearchLoading(false);
